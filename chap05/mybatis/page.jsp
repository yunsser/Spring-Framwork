...
<style>
   ...
   div { display: block; width: fit-content; }
	a { background-image: url(""); text-decoration: none;}
	a.red{ color: red; }
...
<div id ="header"><label>아이디</label><label>이름</label><label>전화번호</label></div>
<c:forEach var="u" items="${pageInfo.list}">
   <div><label>${u.uid}</label>
   <label><a href="/mb/detail?userid=${u.uid}">${u.name}</a>
...
<c:forEach var="pageNum" items="${pageInfo.navigatepageNums}">
	<c:choose>
		<c:when test="${pageNum==pageInfo.pageNum}">
		 ...
		 <c:otherwise>		 	
			[<a href="/mb/page/${pageNum}">${pageNum}</a>]
		 </c:otherwise>
	</c:choose>
</c:forEach>
